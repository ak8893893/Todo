...
namespace Todo.Abstracts
{
    public abstract class TodoListEditDtoAbstract : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            // 取得使用這個標籤的是哪個類別
            //var dto = validationContext.ObjectInstance; // 因為已經抽象畫了所以不用 用this就可以知道是什麼類別了

            if (this.GetType() == typeof(TodoListPutDto))  // 如果類別是TodoListPutDto 執行下面的二次過濾
            {
                var dtoUpdate = (TodoListPutDto)this;     // 第一行程式碼是將 validationContext.ObjectInstance 強制轉換為 TodoListPutDto 類型的變數，並且將其指定為 dtoUpdate 變數。這裡的 TodoListPutDto 是一個數據傳輸對象，用於更新現有待辦事項列表的名稱。
